...
from irmetrics.topk import recall, precision
...
def my_calc_recall(src, pred, top_k=None):
    "recall@1 is slightly different, strange"
    top_k = TOP_K if top_k is None else top_k
    recall_n = {x: 0 for x in top_k}
    precision_n = {x: 0 for x in top_k}
    ...
    for s, p in zip(src, pred):
        for k in top_k:
            recall_n[k] += recall(s, p, k)
            precision_n[k] += precision(s, p, k)
    recall_n = {k: v / len(pred) for k, v in recall_n.items()}
    precision_n = {k: v / len(pred) for k, v in precision_n.items()}
    return {'recall': recall_n, 'precision': precision_n}
...
def eval_hit_from_file(data_file, retrieval_file,
    ...
    with open(retrieval_file, "r") as f:
        ...
        for k, item in r_d.items():
            if split_flag:
                r = ['_'.join(x.split("_")[:-1])
                     for x in item[retrieval_entry]]
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--chunk-length-file',
                        default="data/tldr/nl.cm/manual_section.tok.json")
    ...
    for cur_func in args.function:
        if cur_func == 0:
            ...
            for k in [1, 10, 30, 50]:
                print(f"top {k}")
                recall_per_manual(args.src_file, args.result_file,
                                  args.chunk_length_file, topk=k)
